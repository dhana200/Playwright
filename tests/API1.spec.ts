...
//     const loginUtil = new APIUtil(apiContext, loginapi);
//     token = await loginUtil.getOrderId();
...
test('API Testcase', async ({page}) => {
    ...
    expect(await page.locator('.ng-star-inserted th[scope="row"]').first().textContent()).toContain(OrderId);
    await page.pause();
});
